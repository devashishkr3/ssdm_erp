...
import { AdmittedStudentTable, EnrolledStudentTable, StudentPreviousAcademicRecordTable, StudentDocumentsTable } from "@/lib/db/schema/student"
import { subjectTable, batchTable, courseTable } from "@/lib/db/schema/department"
import { and, eq, inArray, sql, count } from "drizzle-orm"
import { registerStudentSchema, type RegisterStudentPayload } from "./zod-type/register-student-type"
...
export async function registerStudent(payload: RegisterStudentPayload) {
  ...
  if (!parsed.success) {
    return { success: false as const, message: "Invalid payload: " + parsed.error.message }
  }
  ...
  try {
    ...
    if (existingStudent) {
      return { success: false as const, message: "Student with this UAN has already been admitted" }
    }
    ...
    if (!batch || !batch.course) {
      return { success: false as const, message: "Invalid batch: could not resolve course information" }
    }
    ...
    const sessionYear = batch.academicSession?.name?.match(/\d{4}/)?.[0] || new Date().getFullYear().toString()
    ...
    const data = await db.transaction(async (tx) => {
      ...
      const [admittedStudent] = await tx.insert(AdmittedStudentTable).values({
        UAN: personal.UAN,
      ...
      await tx.insert(StudentPreviousAcademicRecordTable).values({
        ...
        ugObtainedMarks: academic.ugObtainedMarks != null ? Math.round(academic.ugObtainedMarks) : null,
        ugTotalMarks: academic.ugTotalMarks != null ? Math.round(academic.ugTotalMarks) : null,
        ugPercentage: academic.ugPercentage != null ? Math.round(academic.ugPercentage) : null,
      ...
      await tx.insert(StudentDocumentsTable).values({
        studentId: admittedStudent.id,
    ...
    if (error?.code === "23505") {
      return { success: false as const, message: "A student record with these unique details already exists (duplicate UAN, email, or Aadhar)" }
    }

    return { success: false as const, message: error?.message || "Failed to register student" }
  }
}
