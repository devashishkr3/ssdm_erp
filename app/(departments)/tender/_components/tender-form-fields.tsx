...
interface TenderFormFieldsProps {
...
export function TenderFormFields({ form }: TenderFormFieldsProps) {
  const [uploading, setUploading] = useState(false);
  const [uploadError, setUploadError] = useState<string | null>(null);
  const [fileName, setFileName] = useState<string | null>(null);

  const currentDocUrl = form.watch("document");
  ...
  const handleFileUpload = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        if (!json.success || !json.urls?.document) {
  ...
  return (
    <div className="grid gap-3">
      ...
      <Controller
        ...
        render={({ fieldState }) => (
          <Field data-invalid={fieldState.invalid}>
            <FieldLabel required>Document (PDF, max 500KB)</FieldLabel>
            <FieldContent>
              {currentDocUrl ? (
