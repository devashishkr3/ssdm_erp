  ...
  try {
    ...
    const departmentWithCourses = await db.query.departmentTable.findFirst({
      ...
      with: {
        ...
        courses: { where: eq(courseTable.isActive, true) },
      },
    });
    ...
    return { success: true, data: departmentWithCourses };
  } catch (error) {
    return { success: false, message: "Error fetching courses", error: error };
  }
}
